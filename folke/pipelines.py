...
class FolkePipeline:
    ...
    # Database setup
    conn = sqlite3.connect('folke.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `folke`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM folke WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO folke (date, title, link, content)"
